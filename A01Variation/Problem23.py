...
for j in range(len(filenames)):
    ...
    skimage.io.imsave("Figure3{}1.bmp".format(filenames[j]), numpy.clip(u, 0.0, 1.0))
    print("{} DCT finished".format(filenames[j]))
    u, _ = tv.opt_tv_admm(i_degr, sigma, lambdas[j], rhos[j], 1000, 1.0e-3, inv="fft")
    skimage.io.imsave("Figure3{}2.bmp".format(filenames[j]), numpy.clip(u, 0.0, 1.0))
    print("{} FFT finished".format(filenames[j]))
    u, _ = tv.opt_tv_admm(i_degr, sigma, lambdas[j], rhos[j], 1000, 1.0e-3, inv="dst")
    skimage.io.imsave("Figure3{}3.bmp".format(filenames[j]), numpy.clip(u, 0.0, 1.0))
