...
for j, filename in enumerate(filename_list):
    ...
    start = time.time()
    u, ctr = pm.evolve_pm(i_degr, mu, iter_, 100.0, nonl="frac", eps=0.0)
    end = time.time()
    ...
    if j in [0, 1, 3, 10]:
        skimage.io.imsave("Figure05{}0.png".format(filename), numpy.clip(i, 0.0, 1.0))
        skimage.io.imsave("Figure05{}1.png".format(filename), numpy.clip(i_degr, 0.0, 1.0))
        skimage.io.imsave("Figure05{}2.png".format(filename), numpy.clip(u, 0.0, 1.0))
...
with shelve.open("Result") as db:
    db[str((2, 4, "filename"))] = filename_list
    db[str((2, 4, "mu"))] = mu_list
    db[str((2, 4, "time"))] = rt[0]
    db[str((2, 4, "iter"))] = rt[1]
    db[str((2, 4, "origpsnr"))] = rt[2]
    db[str((2, 4, "psnr"))] = rt[3]
    db[str((2, 4, "origssim"))] = rt[4]
    db[str((2, 4, "ssim"))] = rt[5]
