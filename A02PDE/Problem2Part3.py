...
for j, filename in enumerate(filename_list):
    ...
    skimage.io.imsave("Figure04{}0.png".format(filename), numpy.clip(i_degr, 0.0, 1.0))
    for k, iter_ in enumerate(iter_list[j]):
        u, _ = pm.evolve_pm(i_degr, mu, iter_, 0.0, nonl="sqrt")
        rt[0].append((filename, iter_, skimage.measure.compare_psnr(i, u)))
        rt[1].append((filename, iter_, skimage.measure.compare_ssim(i, u)))
        skimage.io.imsave("Figure04{}{}.png".format(filename, k+1), numpy.clip(u, 0.0, 1.0))
...
with shelve.open("Result") as db:
    ...
    db[str((2, 3, "mu"))] = mu_list
    db[str((2, 3, "iter"))] = iter_list
    db[str((2, 3, "psnr"))] = rt[0]
    db[str((2, 3, "ssim"))] = rt[1]
